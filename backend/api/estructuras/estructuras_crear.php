...
if (!$restaurante_id || !$nombre || !$zona_id) {
...
try {
    $db   = getDB();
    $stmt = $db->prepare("
        INSERT INTO Estructuras (restaurante_id, nombre, posX, posY, ancho, alto, color, zona_id, rotacion)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$restaurante_id, $nombre, $posX, $posY, $ancho, $alto, $color, $zona_id, $rotacion]);
